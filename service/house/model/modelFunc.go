...
//把图片的凭证存储到数据中   更新   主图,次图  第一张图片是主图,剩下的图片是次图
...
func GetHouseDetail(houseId, userName string) (house.DetailData, error) {
	...
	//获取当前浏览人信息
	var nowUser User
	if err := GlobalDB.Where("name = ?", userName).Error; err != nil {
